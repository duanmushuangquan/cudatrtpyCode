...
//     //学流的时候，warp_affine_gpu_impl是核函数，异步执行在GPU上。立马返回是否执行成功的结果。具体计算结果要等GPU计算结束后才算正式返回
//     //那为何这里没有设置  cudaStreamhorizen()这类同步函数，而直接cudaMemcpy了呢？
//     //      cudaMemcpy，属于同步版本的内存拷贝.等价于执行cudaMemcpyAsync + cudaDeviceSynchronize
//     //        等价于干了  -》 发送指令，我要复制了的消息,相当于发送cudaMemcpyAsync
